...
- BroadCastHashJoin
  - 概要
    - ノードごとにデータが散らばっている分散環境では通常のHashJoinではノード間でデータをやり取りする操作(シャッフル)が発生する。これがしばしば処理速度低下の原因となる。
...
- Sort Merge Join
  - 各パーティションで結合対象のテーブルを結合キーでソートし, 先頭から結合する。 
  - HashJoinが苦手とする点（ハッシュの衝突，双方が大規模テーブル，等価結合以外）においてはこれが有効。
  - 一方で，ソート処理は大規模データは高コストになりがちでボトルナックとなることが多々。
```sql
  Table A                     Table B
+---------+               +---------+
| key | val|             | key | val|
...
- Shuffle Sort merge Join
  - 概要
    - SortMergeJoinにShuffleを加えたもの
  - 詳細
    - Shuffle
      - 結合キーに基づいて、両方のテーブルのデータを同じキーの値が集まるように分散させる。
    - Sort
      - 各パーティション内で、結合キーに基づいてデータをソート
    - Merge Join
      - ソート済みの2つのデータセットを、マージアルゴリズムで順に見ていき、結合キーが一致するデータを結合。
  - ポイント
    - BroadcastHashJoinと異なり，双方のデータが大きくても利用が可能。
    - テーブル全件をメモリに読み込む必要がにないのでメモリ効率がよい。
    - Shuffleはネットワーク越しのデータ転送が発生するので，パフォーマンスのネックになることが多い。
